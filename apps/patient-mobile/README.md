...
- Google 버튼 → 홈으로 즉시 이동하는 더미 로그인
- 프로젝트 홈, 연구 개요·설명·동의·적합성 미리보기
- 포인트 총점, 활동 내역·랭킹 전환, 프로필·설정·소개 화면
- 화면 이동만 구현하며 인증, 데이터 저장, 실제 동의·적합성 판정, 포인트 지급은 연결하지 않았습니다. 모든 프로필·점수는 샘플입니다.
- `assets/brand/loop.png`는 내장 이미지 생성 도구로 만든 실버·민트 Loop 그래픽입니다.

아래 Pulse 설명은 보관된 이전 PoC에 해당하며 현재 기본 화면에는 표시되지 않습니다.

> 제품 방향은 [2026-09-11 LNS Loop 제품 기준안](../../docs/product/lns-loop-product-brief.md)을 참고하세요. 아래 72시간 Pulse 흐름과 예시 보상은 모션 PoC이며, 100명·12주 UC 연구의 확정 규칙이 아닙니다.
