...
class LoopHealthModule : Module() {
 ...
 override fun definition() = ModuleDefinition {
  ...
  AsyncFunction("requestHistory") { val activity=requireNotNull(appContext.currentActivity);val contract=androidx.health.connect.client.PermissionController.createRequestPermissionResultContract();activity.startActivityForResult(contract.createIntent(activity,setOf("android.permission.health.READ_HEALTH_DATA_HISTORY")),4823) }
  ...
  AsyncFunction("configureAutomatic") Coroutine { id:String,token:String,url:String ->
   ...
   if("android.permission.health.READ_HEALTH_DATA_IN_BACKGROUND" !in granted){
    ...
    activity.startActivityForResult(contract.createIntent(activity,setOf("android.permission.health.READ_HEALTH_DATA_IN_BACKGROUND")),4822)
    error("Allow background health access, then enable automatic sharing again")
   }
  ...
  AsyncFunction("requestPermissions") { names:List<String> ->
   val permissions=names.mapNotNull{types[it]?.let{t->HealthPermission.getReadPermission(t)}}.toTypedArray()
   val activity=requireNotNull(appContext.currentActivity)
   // Health Connect's permission controller uses the standard Android activity-result contract.
   val contract=androidx.health.connect.client.PermissionController.createRequestPermissionResultContract()
   activity.startActivityForResult(contract.createIntent(activity,permissions.toSet()),4821)
  }
